...
public:
	...
	template <class T>
	static bool MatToImage(const cv::Mat &im,T*& pImagePlane);
	template <class T>
	static cv::Mat ImageToMat(const T* pImagePlane,int width,int height, int nchannels,ImageType imtype = standard);

};

template <class T>
bool ImageIO::MatToImage(const cv::Mat &im, T *&pImagePlane)
{
...
template <class T>
cv::Mat ImageIO::ImageToMat(const T* pImagePlane,int width,int height, int nchannels,ImageType imtype)
{
	cv::Mat im;
	switch(nchannels){
		case 1:
